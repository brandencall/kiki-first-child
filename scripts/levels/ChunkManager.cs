...
public partial class ChunkManager : Node2D
{
	...
	/// <summary>
	/// Returns the world-space center position of a chunk.
	/// </summary>
	public Vector2 GetChunkCenter(Vector2I chunkCoor)
	{
		// Convert chunk coordinate to the world-space origin of that chunk,
		// then offset by half a chunk to reach its center.
		float halfChunkWorld = (chunkSize / 2f) * tileSize;
		float worldX = chunkCoor.X * chunkSize * tileSize + halfChunkWorld;
		float worldY = chunkCoor.Y * chunkSize * tileSize + halfChunkWorld;
